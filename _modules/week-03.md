---
title: Week 3 — Arrays and Tables
class: Berkeley
status: Active
---
Mon Sep 8  
: **3**{: .label .label-lecture} {{ site.data.lec[3].name }}  
    : [Slides]({{ site.data.lec[3].slides }})
    &#8226;  [Blank Demo]({{ site.data.lec[3].demo }})

Wed Sep 10  
: **4**{: .label .label-lecture} {{ site.data.lec[4].name }}  
    : [Slides]({{ site.data.lec[4].slides }})
    &#8226;  [Blank Demo]({{ site.data.lec[4].demo }}) 
    &#8226;  [Sheet]({{ site.data.lec[4].demo-sheet }})
: **Reading 3**{: .label .label-reading} [{{ site.data.reading[3].name }}]({{site.data.reading[3].link}})  
    : (due Thu 9/11)
: **Homework 2**{: .label .label-homework} [{{ site.data.hw[2].name }}]({{site.data.hw[2].link}})
    : (due Thu 9/18, 8pm)  

Fri Sep 12  
: **Discussion 3**{: .label .label-discussion} {{ site.data.disc[3].name }}
   : [Slides]({{ site.data.disc[2].slides }}) &#8226; [Worksheet]( {{ site.data.disc[3].worksheet }}) &#8226; [Solutions]( {{ site.data.disc[3].solution }})
: **Lab 3**{: .label .label-lab} [{{ site.data.lab[3].name }}]({{site.data.lab[3].link}})
    : (due Fri 9/12)  



