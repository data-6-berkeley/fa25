---
title: Week 2 — Computation of Rates, Python Names
class: Berkeley
status: Active
---
Mon Sep 1  
: **Holiday**{: .label} No class

Wed Sep 3  
: **2**{: .label} {{ site.data.lec[2].name }}  
    : [Slides]({{ site.data.lec[2].slides }})
    &#8226;  [Demo]({{ site.data.lec[2].demo }})
: **Reading 2**{: .label .label-reading} [{{ site.data.reading[2].name }}]({{site.data.reading[2].link}}) 
    : (due Thu 9/4, 11:59pm)
: **Homework 1**{: .label .label-homework} [{{ site.data.hw[1].name }}]({{site.data.hw[1].link}})
    : (due Wed 9/10, 5pm)

Fri Sep 5  
: **Discussion 2**{: .label .label-discussion} {{ site.data.disc[2].name }}
    : [Slides]({{ site.data.disc[2].slides }}) &#8226; [Worksheet]( {{ site.data.disc[2].worksheet }}) &#8226; [Solutions]( {{ site.data.disc[2].solution }})
   <!--: Slides &#8226; Worksheet &#8226; Solution -->
: **Lab 2**{: .label .label-lab} [{{ site.data.lab[2].name }}]({{site.data.lab[2].link}})
    : (due Fri 9/5)