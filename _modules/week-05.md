---
title: Week 5 Quiz, Visualizations
class: Berkeley
status: Active
---
Mon Sep 22  
: **Quiz 1**{: .label .label-exam} {{ site.data.quiz[1].name }} 

Wed Sep 24  
: **7**{: .label .label-lecture} {{ site.data.lec[7].name }}  
    : [Slides]({{ site.data.lec[7].slides }})
    &#8226;  [Blank Demo]({{ site.data.lec[7].demo }})
: **Reading 5**{: .label .label-reading} [{{ site.data.reading[5].name }}]({{site.data.reading[5].link}}) 
    : (due Thu 9/25)
: **Homework 1**{: .label .label-homework} Resubmit ([details](https://docs.google.com/presentation/d/1kHypBMRJ_HCOeC543VrwRDQnQI0_oogieRPN6GSlUHw/edit?slide=id.g38ce788ed61_0_7#slide=id.g38ce788ed61_0_7){: target="\_blank"})
    : (due Wed 10/1 5pm)

Fri Sep 26  
: **Discussion 5**{: .label .label-discussion} {{ site.data.disc[5].name }}  
    : [Slides]({{ site.data.disc[5].slides }}) &#8226; [Worksheet]( {{ site.data.disc[5].worksheet }}) &#8226; [Solutions]( {{ site.data.disc[5].solution }})
: **Lab 5**{: .label .label-lab} [{{ site.data.lab[5].name }}]({{site.data.lab[5].link}})
    : (due Fri 9/26)  
: **Survey**{: .label .label-homework} [{{ site.data.survey[1].name }} ]( {{site.data.survey[1].link }})
    : (due Fri 9/26 **5pm**)
